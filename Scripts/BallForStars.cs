...
public class BallForStars : BallBuyer
{
    ...
    {
        if (_isBought == false && _price <= GameData.Instance.YandexData.BlackStars)
        {
            _isBought = true;
            _imageToBuy.gameObject.SetActive(false);
            GameData.Instance.BuyBallByStars(GameData.Instance.YandexData.BlackStars - _price);
            GameData.Instance.ResetBallInfo(_ballImage.sprite, _isBought);
